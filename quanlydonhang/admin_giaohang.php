...
<?php 
    ...
    if(isset($_POST['xacnhan'])) {
        $sql_count_donhang=mysqli_query($conn,"select count(id_donhang) as sl_donhang from donhang");
        $row_count_donhang=mysqli_fetch_array($sql_count_donhang);
        for($i=0;$i<$row_count_donhang['sl_donhang'];$i++){
            if(isset($_POST['check'][$i]) && $_POST['check'][$i] == "true"){
               $id_donhang=$_POST['id_donhang'][$i];
               $sql_update_donhang=mysqli_query($conn,"update donhang set trangthai='3' where id_donhang='$id_donhang'");
            }
        }
    }
?>
...
<header>
    ...
    <main>
        <h2 style="text-align: center;">Admin Xác Nhận Hàng Đang Được Giao</h2><br>
